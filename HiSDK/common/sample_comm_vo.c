...
HI_S32 SAMPLE_COMM_VO_StartDev(VO_DEV VoDev, VO_PUB_ATTR_S *pstPubAttr)
{
    HI_S32 s32Ret = HI_SUCCESS;
    //printf("-----------------dev:%d\n", VoDev);
    s32Ret = HI_MPI_VO_SetPubAttr(VoDev, pstPubAttr);//频输出设备的公共属性。
...
HI_S32 SAMPLE_Preview_VO_StartChn(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode)
{
    HI_S32 i;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WndNum = 0;
...
HI_S32 SAMPLE_Playback_VO_StartChn(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WndNum = 0;
    HI_U32 u32Square = 0;
    ...
    if(u32WndNum==1)
    {
        ...
        stChnAttr.stRect.s32X       = 2;
        stChnAttr.stRect.s32Y       = 2;
        stChnAttr.stRect.u32Width   = 1682; 
        stChnAttr.stRect.u32Height  = 918;
    }
...
HI_S32 SAMPLE_COMM_VO_StartFullSreen(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode,HI_S32 s32ChnNum)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WndNum = 0;
    HI_U32 u32Square = 0;
    ...
    // for (i=0; i<u32WndNum; i++)
    {
        ...
        stChnAttr.stRect.s32Y       = ALIGN_BACK((u32Height/u32Square) * (0/u32Square), 2);
        stChnAttr.stRect.u32Width   = ALIGN_BACK(u32Width/u32Square, 2);
        stChnAttr.stRect.u32Height  = ALIGN_BACK(u32Height/u32Square, 2);
        stChnAttr.u32Priority       = 0;
        stChnAttr.bDeflicker        = HI_FALSE;

        s32Ret = HI_MPI_VO_SetChnAttr(VoLayer, s32ChnNum, &stChnAttr);
        if (s32Ret != HI_SUCCESS)
        {
            printf("%s(%d):failed with %#x!\n",\
                   __FUNCTION__,__LINE__,  s32Ret);
...
HI_S32 SAMPLE_COMM_VO_StopChn(VO_LAYER VoLayer, HI_S32 s32ChnNum )
{
    HI_S32 s32Ret = HI_SUCCESS;
    s32Ret = HI_MPI_VO_DisableChn(VoLayer, s32ChnNum);
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_PRT("failed with %#x!\n", s32Ret);
        return HI_FAILURE;
...
HI_S32 SAMPLE_COMM_VO_HdmiStart(VO_INTF_SYNC_E enIntfSync)
{
    ...
    //  增加了enVideoFormat 的初始化---by hejunying
